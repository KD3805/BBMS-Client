...
const useDonationApi = () => {
  ...
  const updateDonationApi = async (formData, DonationID) => {
    try {
      console.log("Payload being sent in Donation Request (update):", formData); // Debugging
      const response = await axiosInstance.put(`/Donation/${DonationID}`, {
        id: DonationID,
        donationModel: formData,
      });
      return response.status === 200;
    } catch (error) {
  ...
  const updateDonationStatusApi = async (DonationID, NewStatus) => {
    try {
      const response = await axiosInstance.put(`/Donation/updateStatus/${DonationID}`, {
        DonationID,
        NewStatus
      });
      return response.status === 200;
