...
const DonorSignup = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        ...
        const isDonorExist = await checkDonorExistApi(formData.Email);
        if (isDonorExist) {
          toast.error("Donor with given email already exists.");
          return;
        }
  ...
  // Handle OTP verification and donor registration
  const handleOtpSubmit = async (otp) => {
    try {
      ...
      if (otpVerified) {
        ...
        Swal.fire("Good job!", "Donor Registered Successfully", "success");
